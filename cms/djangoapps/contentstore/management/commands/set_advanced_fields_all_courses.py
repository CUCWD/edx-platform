...
class Command(BaseCommand):
    ...
    Invoke with:
        ...
        --fields_json '{"invitation_only": {"value": "true"}}' --user edx@example.com
        --course course-v1:ORG+COURSENUMBER+COURSERUN --force-update-catalog-visibility-about
        ...
        python manage.py cms --settings=devstack_docker set_advanced_fields_all_courses \
        --fields_json '{"invitation_only": {"value": "true"},
            "certificates_display_behavior": {"value": "early_no_info"}}'
        --user edx@example.com --course course-v1:ORG+COURSENUMBER+COURSERUN --force-update-catalog-visibility-about

        # All Courses
        # Set the following advanced settings:
        # `catalog_visilibilty` to `about`
        python manage.py cms --settings=devstack_docker set_advanced_fields_all_courses \
        --fields_json '{}' --user edx@example.com
        --course course-v1:ORG+COURSENUMBER+COURSERUN --force-update-catalog-visibility-about
    """
    ...
    def handle(self, *args, **options):
        ...
        self._set_courses_advanced_fields(update_fields, options['user_email'], options['course_id'], int(options['limit_bulk_update_operation']))  # pylint: disable=line-too-long
