...
class BigCommerceAppCallbacks():
    ...
    These callback URLs will be called during the OAuth registration process, or when the user
    uninstalls your app. Importantly, communication with these endpoints must be done over HTTPS.
    """

    @classmethod
    def auth(cls, request):
        """
        The GET request to your app’s auth callback URL contains a temporary code that can be
        exchanged for a permanent access_token. It also includes a unique value that identifies
        the store installing or updating your app, as well as authorized scopes.

        https://developer.bigcommerce.com/api-docs/apps/guide/auth#receiving-the-get-request
        """
        LOGGER.info(
            "Initiated single-click app `auth_callback` from BigCommerce"
        )
        ...
        auth_redirect = \
            f"{platform_lms_url()}{reverse('bigcommerce_app_callbacks:auth')}".rstrip("/")

        LOGGER.info(
            "The `auth_redirect` is %s",
            auth_redirect
        )
        ...
        store.scope = scope
        store.save()
        # If the app was installed before, make sure the old admin user is no longer marked
        # as the admin
        if not store_created:
            old_store_admin_user = StoreAdminUser.objects.filter(
                store__id=store.id,
                is_admin=True
            ).first()
        ...
        store_admin_user, store_admin_user_created = \
            StoreAdminUser.objects.get_or_create(store_id=store.id, bc_admin_user_id=admin_user.id)
        if store_admin_user_created:
            store_admin_user.is_admin = True
        store_admin_user.save()

        response = redirect(
            reverse("bigcommerce_app_single_click:index") +
            f'?bc_storeadminuserid={store_admin_user.bc_admin_user.bc_id}'
        )

        # Todo: This doesn't work at the moment.
        # response.set_cookie(
        #   'bc_storeadminuserid', store_admin_user.bc_admin_user.bc_id, secure=True, max_age=1000
        # )
    ...
    def load(cls, request):
        """
        BigCommerce sends a GET request to your app’s load URL when the store owner or user clicks
        to load the app.

        The steps to handle this callback are as follows:

        1. Verify the signed payload(https://developer.bigcommerce.com/api-docs/apps/guide/callbacks#verifying-the-signed-payload).  # lint-amnesty, pylint: disable=line-too-long
        2. Identify the user (https://developer.bigcommerce.com/api-docs/apps/guide/callbacks#identifying-users).  # lint-amnesty, pylint: disable=line-too-long
        ...
        """
        LOGGER.info(
            "Initiated single-click app `load_callback` from BigCommerce"
        )
        ...
        try:
            ...
            LOGGER.error(
                "Could not get access token from BigCommerce in `auth_callback`%s",
                excep
            )
            return internal_server_error(excep)
    ...
    def uninstall(cls, request):
        """
        BigCommerce sends a GET request to your app’s uninstall URL when the store owner clicks
        ...
        """
        LOGGER.info(
            "Initiated single-click app `uninstall_callback` from BigCommerce"
        )
    ...
    def remove_user(cls, request):
        """
        BigCommerce sends a GET request to your app’s remove user callback when a store admin
        ...
        """
        LOGGER.info(
            "Initiated single-click app `remove_user_callback` from BigCommerce"
        )
        ...
        if admin_user is not None:
            store_admin_user = StoreAdminUser.objects.filter(
                bc_admin_user__id=bc_store_admin_user_id,
                store__id=store.id
            ).first()
            store_admin_user.delete()
