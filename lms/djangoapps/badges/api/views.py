...
from rest_framework.exceptions \
    import APIException, AuthenticationFailed  # pylint: disable=unused-import
    ...
    **Response Values**
        ...
        * badge_class: The badge class the assertion was awarded for. Represented as an object
          with the following fields:
            ...
            * course_id: The course key of the course this badge is scoped to, or null if it isn't
                scoped to a course.
        ...
        * image_url: The baked assertion image derived from the badge_class icon-- contains metadata
                about the award in its headers.
        * assertion_url: The URL to the OpenBadges BadgeAssertion object, for verification by
                compatible tools and software.

    **Params**

        * slug (optional): The identifier for a particular badge class to filter by.
        * issuing_component (optional): The issuing component for a particular badge class to filter
            by (requires slug to have been specified, or this will be ignored.) If slug is provided
            and this is not, assumes the issuing_component should be empty.
        * course_id (optional): Returns assertions that were awarded as part of a particular course.
            If slug is provided, and this field is not specified, assumes that the target badge has
            an empty course_id field. '*' may be used to get all badges with the specified slug,
            issuing_component combination across all courses.
            ...
            "results": [
                {
                    "badge_class": {
                        ...
                        "image": "http://example.com/media/badge_classes/badges/special_xdpqpBv_9FYOZwN.png"  # pylint: disable=line-too-long
                    },
                    "image_url": "http://badges.example.com/media/issued/cd75b69fc1c979fcc1697c8403da2bdf.png",  # pylint: disable=line-too-long
    ...
    def get_queryset(self):
        ...
        if provided_course_id == '*':
            ...
            try:
                ...
                raise InvalidCourseKeyError from invalid_key_error
        elif 'slug' not in self.request.query_params:
            # Need to get all badges for the user.
            course_id = None
        else:
            # Django won't let us use 'None' for querying a ForeignKey field. We have to use this
            # special 'Empty' value to indicate we're looking only for badges without a course key
            # set.
...
class BadgeProgressViewMixin(DeveloperErrorViewMixin):
    ...
    def _get_single_user_badge_progress(self, course_key, username=None):
        ...
        # Traverse through all course chapter blocks and check to see if a user has
        # a badge assertion.
    ...
    def _make_badge_progress_response(self, course_key, block_event_badge_config, block_event_assertion):  # pylint: disable=line-too-long
        """
...
class CourseBadgeProgressListView(BadgeProgressViewMixin, APIView):
    ...
    ** Returns **
                ...
                "progress": [
                    {
                        ...
                        "assertion": {
                            ...
                            "issuer": {
                                ...
                                "openBadgeId": "https://api.badgr.io/public/issuers/npqlh0acRFG5pKKbnb4SRg",  # pylint: disable=line-too-long
                                "name": "EducateWorkforce",
                                "image": "https://media.us.badgr.io/uploads/issuers/issuer_logo_77bb4498-838b-45b7-8722-22878fedb5e8.svg",  # pylint: disable=line-too-long
                                "email": "cucwd.developer@gmail.com",
                                "description": "An online learning solution offered with partnering 2-year colleges to help integrate web and digital solutions into their existing courses. The platform was designed by multiple instructional design, usability, and computing experts to include research-based learning features.",  # pylint: disable=line-too-long
                                "url": "https://ew-localhost.com",
                            },
    ...
    permission_classes = (
        permissions.JWT_RESTRICTED_APPLICATION_OR_USER_ACCESS,
        permissions_openedx.IsMasterCourseStaffInstructor
    )

    # required_scopes = ['badges:read']

    def get(self, request, course_id=None, *args, **kwargs):  # pylint: disable=missing-function-docstring,disable=keyword-arg-before-vararg,disable=unused-argument
        ...
        try:
            ...
            ) from invalid_key_error

        if not CourseOverview.get_from_id(course_key):
            raise self.api_error(
                status_code=status.HTTP_404_NOT_FOUND,
                developer_message=f"Requested grade for unknown course {course_id}",
        ...
        #         raise self.api_error(
        #             status_code=status.HTTP_404_NOT_FOUND,
        #             developer_message='The user matching the requested username is not enrolled in this course',  # pylint: disable=line-too-long
...
class UserBadgeProgressListView(BadgeProgressViewMixin, APIView):
    ...
    **Response Values**
        ...
        * assertion: The assertion this badge progress is scoped to. Represented as an object with
            the following fields:
            * image_url: The baked assertion image derived from the badge_class icon-- contains
                metadata about the award in its headers.
            * assertion_url: The URL to the OpenBadges BadgeAssertion object, for verification by
                compatible tools and software.
    ...
    def get(self, request, course_id=None, username=None, *args, **kwargs):  # pylint: disable=unused-argument,disable=keyword-arg-before-vararg
        ...
        try:
            ...
            raise self.api_error(
                ...
                developer_message='The user matching the requested username does not exist.',
                error_code='user_does_not_exist'
            ) from user_model_not_exist
        except CourseEnrollment.DoesNotExist as course_enrollment_not_exist:
            raise self.api_error(
                status_code=status.HTTP_404_NOT_FOUND,
                developer_message='The user matching the requested username is not enrolled in this course',  # pylint: disable=line-too-long
                error_code='user_not_enrolled'
            ) from course_enrollment_not_exist
        # else:
        #     # If no username passed, get paginated list of grades for all users in course
        #     return Response(self._get_course_badge_progress(course_key))
        ...
        #             "badge_class": {
        #                 "slug": block_event_badge_config.badge_class.slug,
        #                 "issuing_component": block_event_badge_config.badge_class.issuing_component,  # pylint: disable=line-too-long
        #                 "display_name": block_event_badge_config.badge_class.display_name,
        #                 "course_id": CourseKeyFieldSerializer(source='course_key').\
        #                     to_representation(block_event_badge_config.badge_class.course_id),
        #                 "description": block_event_badge_config.badge_class.description,
