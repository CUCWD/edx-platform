...
class BadgeProgressViewMixin(DeveloperErrorViewMixin):
    ...
    def _make_badge_progress_response(self, course_key, block_event_badge_config, block_event_assertion):
        ...
        return {
            ...
            "block_display_name": self.course_section_mapping.get(UsageKeyField(source='usage_key').to_representation(block_event_badge_config.usage_key), '').get('display_name', ''),
            "block_order": self.course_section_mapping.get(UsageKeyField(source='usage_key').to_representation(block_event_badge_config.usage_key), '').get('block_order', ''),
...
class CourseBadgeProgress(BadgeProgressViewMixin, GenericAPIView):
    """
    ** Use cases **
...
def _user_badge_progress(request, username, course_id):
    """
    Get user badge progress for a specific course.
    """
        ...
        progress_to_show.append(
            {
                ...
                "block_display_name": course_section_mapping.get(
                    UsageKeyField(source='usage_key').to_representation(blockEventBadgeConfig.usage_key), '').get(
                    'display_name', ''),
                ...
                "badge_class": {
                    ...
                    "course_id": CourseKeyField(source='course_key').to_representation(
                        blockEventBadgeConfig.badge_class.course_id),
                ...
                "assertion": {
                    "issuedOn": (block_event_assertion.data.get('issuedOn', '') if hasattr(block_event_assertion,
                                                                                           'data') else ""),
                    "expires": (block_event_assertion.data.get('expires', '') if hasattr(block_event_assertion,
                                                                                         'data') else ""),
                    "revoked": (block_event_assertion.data.get('revoked', False) if hasattr(block_event_assertion,
                                                                                            'data') else False),
                    "image_url": (block_event_assertion.image_url if block_event_assertion else ""),
                    "assertion_url": (block_event_assertion.assertion_url if block_event_assertion else ""),
                    "entityId": (block_event_assertion.data.get('entityId', '') if hasattr(block_event_assertion,
                                                                                           'data') else ""),
                    "recipient": {
                        "plaintextIdentity": (block_event_assertion.user.email if block_event_assertion else ""),
                    },
